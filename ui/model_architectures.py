...
class Data3DSegToSeg(Dataset):
    def __init__(self, file_dir, seg_dir, indices):
        ...
        self.seg_t1_list =  [x.split("_T1w")[0] + "_seg_T1w.npy" for x in [y[0] for y in self.file_list]]
        self.seg_t2_list =  [x.split("_T2w")[0] + "_seg_T2w.npy" for x in [y[1] for y in self.file_list]]
        self.seg_list = [(x, y) for x, y in zip(self.seg_t1_list, self.seg_t2_list)]
    ...
    def __getitem__(self, idx):
        ...
        seg_t1_path = os.path.join(self.seg_dir, self.seg_list[idx][0])
        seg_t1 = np.load(seg_t1_path)
        
        seg_t2_path = os.path.join(self.seg_dir, self.seg_list[idx][1])
        seg_t2 = np.load(seg_t2_path)
        
        return np.reshape(seg_t1, (9, 256, 256, 256)), np.reshape(seg_t2, (9, 256, 256, 256))
