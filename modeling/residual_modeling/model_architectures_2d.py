...
class DataSeg2Seg(Dataset):
    def __init__(self, seg_dir, pred_dir, indices, seg_num, batch_size):
    ...
    def __getitem__(self, idx):
        ...
        seg = np.reshape(np.moveaxis(seg, [0, 1, 2], [1, 2, 0]), (256, 1, 256, 256))
        
        pred_path = os.path.join(self.pred_dir, self.file_list[idx][1])
        pred = np.load(pred_path)[self.seg_num]
        pred = np.reshape(np.moveaxis(pred, [0, 1, 2], [1, 2, 0]), (256, 1, 256, 256))
        
        batch_split = int(256 / self.batch_size)
        seg_batched = np.split(seg, batch_split)
        pred_batched = np.split(pred, batch_split)
        
        return [(y, x) for x, y in zip(seg_batched, pred_batched)]

    
class DataSeg2SegUnbatched(Dataset):
