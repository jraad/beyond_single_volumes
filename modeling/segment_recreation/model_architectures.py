...
class Data3D(Dataset):
    def __init__(self, file_dir, indices, t2_only=False):
...
class Data3DSegT1T2_toT2Seg(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return np.stack([t1, t2]), np.reshape(seg, (9, 256, 256, 256))
...
class Data3DSegToSeg(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return np.reshape(seg, (9, 256, 256, 256))

class Data3DSegToSegT1(Dataset):
    def __init__(self, file_dir, seg_dir, indices):
        self.file_dir = file_dir
        self.seg_dir = seg_dir
        self.t1_list = sorted([x for x in os.listdir(self.file_dir) if x.endswith("T1w.npy")])
        self.t2_list = sorted([x for x in os.listdir(self.file_dir) if x.endswith("T2w.npy")])
        self.file_list = [y for x, y in enumerate(zip(self.t1_list, self.t2_list)) if x in indices]
        self.seg_list =  [x.split("_T1w")[0] + "_seg_T1w.npy" for x in [y[0] for y in self.file_list]]
...
class Data3DSingleSegT2(Dataset):
    def __init__(self, file_dir, seg_dir, indices, segment):
        self.file_dir = file_dir
        self.seg_dir = seg_dir
    ...
    def __getitem__(self, idx):
        ...
        seg_path = os.path.join(self.seg_dir, self.seg_list[idx])
        seg = np.load(seg_path)
        seg = seg[self.segment]
        
        return np.reshape(t2, (1, 256, 256, 256)), np.reshape(seg, (1, 256, 256, 256))


class Data3DSingleSegToSingleSeg(Dataset):
    def __init__(self, file_dir, seg_dir, indices, segment):
        self.file_dir = file_dir
        self.seg_dir = seg_dir
        self.segment = segment
    ...
    def __getitem__(self, idx):
        ...
        return np.reshape(seg, (1, 256, 256, 256))


class Data3DSingleSegToSingleSegT1(Dataset):
    def __init__(self, file_dir, seg_dir, indices, segment):
        self.file_dir = file_dir
        self.seg_dir = seg_dir
        self.segment = segment
